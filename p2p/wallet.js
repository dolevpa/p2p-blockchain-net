...
var t = topology(myIp, peerIps).on("connection", (socket, peerIp) => {
    ...
    const sendSingleTransaction = (socket) => {
        ...
        if (transactions.transactions[index] !== undefined && transactions.transactions[index].fromAddress === name) {
            log(transactions.transactions[index]);
            log(pkMap.get(name));
    ...
    // if ((name === "bob") & first) {
    //     setTimeout(() => setInterval(() => sendSingleTransaction(socket), 2000), 4000);
    //     first = false;
    // } else
